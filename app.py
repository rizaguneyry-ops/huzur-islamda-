...
# --- 1. AYARLAR VE TASARIM (Madde 13: Arka Plan ve Görsellik) ---
...
# --- 2. VERİ SETLERİ (Madde 8: 99 Esma, Madde 10: 10 Sure Tam Metin) ---
...
SURELER_TAM = {
    ...
    "İhlas Suresi": "Bismillahirrahmânirrahîm. Kul hüvallâhü ehad. Allâhü's-samed. Lem yelid ve len yûled. Ve lem yekün lehû küfüven ehad.",
...
# --- 3. SIDEBAR (Madde 5, 6, 7, 8, 9, 10, 11, 12) ---
with st.sidebar:
    ...
    st.info("🧭 **Kıble Yönü:** 147° (Güneydoğu)")
...
try:
    ...
    while True:
        ...
        # Madde 2: Bir Sonraki Vakte Geri Sayım
        hedef_ad = "İmsak"; hedef_vakit = None
        for ad, saat_str in v_map.items():
            t = datetime.strptime(saat_str, "%H:%M").replace(year=simdi.year, month=simdi.month, day=simdi.day)
            if t > simdi: hedef_ad = ad; hedef_vakit = t; break
        if not hedef_vakit:
            hedef_vakit = datetime.strptime(v_map["İmsak"], "%H:%M").replace(year=simdi.year, month=simdi.month, day=simdi.day) + timedelta(days=1)
        ...
        with govde.container():
            ...
            # Bugünün Vakitleri
            st.markdown("### 🕌 Bugünün Namaz Vakitleri")
            kollar = st.columns(6)
            ...
            # Madde 3 & 4: Haftalık Vakit (Tam 6 Vakit ve Bugünden İtibaren)
            st.divider()
            st.subheader("🗓️ 7 Günlük Tam Takvim")
            ...
            for i in range(7):
                gun = calendar_data[simdi.day - 1 + i]
    ...
    st.error("Veriler güncelleniyor, lütfen bekleyin...")
    time.sleep(3)
    st.rerun()