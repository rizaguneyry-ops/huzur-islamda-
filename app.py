...
st.markdown(f"""
    ...
    /* ESMAÜL HÜSNA ÖZEL KUTU (Kaydırılabilir) */
    .esma-container {{
...
esma_99 = [
    ...
    ("Ed-Dârr", "Zarar veren (hikmeti gereği)."), ("En-Nâfi", "Fayda veren."),
...
try:
    ...
    # VAKİTLER
    c1 = st.columns(3)
    for i in range(3): with c1[i]: st.markdown(f'<div class="vakit-card"><div style="font-size:12px; opacity:0.7;">{v_order[i][0]}</div><div class="vakit-time">{v_order[i][1]}</div></div>', unsafe_allow_html=True)
    c2 = st.columns(3)
    for i in range(3, 6): with c2[i-3]: st.markdown(f'<div class="vakit-card"><div style="font-size:12px; opacity:0.7;">{v_order[i][0]}</div><div class="vakit-time">{v_order[i][1]}</div></div>', unsafe_allow_html=True)
...
with col1:
    ...
    if st.button("➕ ZİKİR ÇEK"): st.session_state.zk += 1; st.rerun()
